...
class barcode_marketing_check(models.Model):
	...
	def check_mobile_coupon(self, 
		...
		data2 = {}
		try:
			# scanned_coupon = line_ids.sudo().search([('name','=',coupon), ('flag','=', True)])
			# if scanned_coupon:
			new_coupon = line_ids.sudo().search([('name','=',coupon), ('flag','=', False)])
			already_scanned_coupon = line_ids.sudo().search([('name','=',coupon), ('flag','=', True), 
										('second_flag','=', False),	('barcode_marketing_id','in', (10,11))])
			...
			if new_coupon:
				...
				# raise Warning(_('Coupon already Scanned'))
			...
			return 'Invalid Coupon. Coupon Not Present'
			# raise Warning(_('Invalid Coupon. Coupon Not Present'))
	...
	def onchange_name(self):
		...
		if self.barcode :
			...
			if self.recheck_bool:
				barcode_ids = self.env['barcode.marketing.line'].sudo().search([('name','=',self.barcode),('recheck_bool','=',False)])
				if barcode_ids:
				...
				barcode_ids = self.env['barcode.marketing.line'].sudo().search([('name','=',self.barcode),('flag','=',False)])
				if barcode_ids:
					list_qrcode.append(barcode)
				else:
