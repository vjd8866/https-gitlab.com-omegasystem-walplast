...
class payment_automation_report(models.TransientModel):
    _name = 'payment.automation.report'
    _description = "Approved Invoice Report"


    name = fields.Char(string="payment_automation_report")
    ...
    def print_report(self):
        ...
        if self.date_from and self.date_to:
            ...
            worksheet.write_merge(0, 1, 0, 15, self.name ,main_style)
            row_index = 2
        ...
        #     # https://github.com/python-excel/xlwt/blob/master/xlwt/Style.py
            ...
            if not self.all_records:
                bank_payment_id = bank_payment_id.sudo().search([('state','=', 'approved')])
                    ...
                    if rec:
                        ...
                        if issuance_lines:
                            ...
                            for record in issuance_lines:
                                ...
                                worksheet.write(row_index, 7,count2, base_style_yellow )
                                worksheet.write(row_index, 8, record.value_date , base_style_yellow )
                                worksheet.write(row_index, 9, record.documentno, base_style_yellow )
                                worksheet.write(row_index, 10, record.customercode, base_style_yellow )
                                worksheet.write(row_index, 11, record.beneficiary_name, base_style_yellow )
                                worksheet.write(row_index, 12, str(abs(record.totalamt)), base_style_yellow )
                                worksheet.write(row_index, 13, str(abs(record.allocatedamt)), base_style_yellow )
                                worksheet.write(row_index, 14, str(abs(record.unallocated)), base_style_yellow )
                                worksheet.write(row_index, 15, record.duedays, base_style_yellow )
        ...
        return {
            ...
            'res_model': 'payment.automation.report',
            'view_mode': 'form',
            'view_type': 'form',
            'res_id': self.id,
            'target': 'new',
