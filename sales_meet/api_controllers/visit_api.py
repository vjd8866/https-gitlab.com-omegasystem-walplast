...
class WpVisitController(http.Controller):
    ...
    @http.route('/wmvdapi/create_visit', methods=["POST"], type='json', auth='public')
    def create_visit(self, visit_id=None, user_id=None, lead_id=None, retailer_id=None, distributor_id=None, stage_id=None,
         date=None, started_at=None, latitude=None, longitude=None, description=None, address=None,
        visit_type=None, location_type=None):
        print("------------/wmvdapi/create_visit -----------", visit_id, user_id, lead_id, retailer_id, distributor_id, 
            stage_id, date, started_at, latitude, longitude, description, address)
        ...
        meet_name = "Meeting With " + (lead.name if lead_id else  partner.name if distributor_id else \
            retailer.name if  retailer_id else '')
    ...
    @http.route('/wmvdapi/create_home_visit', methods=["POST"], type='json', auth='public')
    def create_home_visit(self, visit_id=None, user_id=None, meeting_date=None, started_at=None,
        ...
        print("------------/wmvdapi/create_home_visit -----------", user_id)
        users_rec = request.env['res.users'].sudo().search([('id', '=', user_id),('active', '=', True)], limit=1).name
        meet_name = "Home Location"
        homevisit = request.env['calendar.event'].sudo().search([('user_id', '=', user_id),
            ('expense_date', '=', meeting_date),
            ('ishome', '=', True)])

        if homevisit:
            return {'success': None, 'error':'Home Location Already Punched for today'}
    ...
    @http.route('/wmvdapi/get_user_visits', auth='public', methods=["POST"], type='json')
    def get_user_visits(self, user_id=None, date=None):
        print("------------/wmvdapi/get_user_visits -----------")
        if date:
            domain = [('user_id', '=', user_id),('meeting_type','=','check-in'),('expense_date','=',date.encode('ascii', 'ignore'))]
        else:
        ...
        if calendar_rec:
            for res in calendar_rec:
                ...
                vals = {
                        ...
                        'location_type': res.location_type or '',
                }
    ...
    @http.route('/wmvdapi/get_crm_activity', auth='public', methods=["POST"], type='json')
    ...
    @http.route('/wmvdapi/get_crm_stage', auth='public', methods=["POST"], type='json')
    ...
    @http.route('/wmvdapi/get_salesperson_lead', auth='public', methods=["POST"], type='json')
...
#        result = request.env['crm.lead'].search([('state_id', '=', state_id),('state_id', '!=', False),]).read(['id','name'])
    ...
    @http.route('/wmvdapi/get_assigned_retailer_list', methods=["POST"], type='json', auth='public')
    ...
    @http.route('/wmvdapi/get_salesperson_distributor_list', methods=["POST"], type='json', auth='public')
    ...
    # @http.route('/wmvdapi/edit_visit', methods=["POST"], type='json', auth='user')
    @http.route('/wmvdapi/edit_visit', methods=["POST"], type='json', auth='public')
    def edit_visit(self, visit_id=None, user_id=None, vals=None):
        print("------------/wmvdapi/edit_visit -----------", user_id)

        if vals:
            calendar = request.env['calendar.event'].sudo().search([('mobile_id', '=', visit_id)])

            if calendar:
                for res in vals:
                    print("---------fffffffffff----------", res)
                    crec = calendar.sudo().write(res)
                    ...
                    vals = {
                        "id": visit_id,
                        "stage_id": crec.stage_id.id or None,
                        "stage": crec.stage_id.name or '',
                        "activity_id": crec.activity_id.id or 0,
                        "activity": crec.activity_id.name or '',
                        "time": crec.started_at,
                        "user_id": user_id,
                        "user": crec.user_id.name,
                        "name": meet_name  or '',
                        'distributor_id': partner.id if distributor_id else None,
                        'lead_id': lead.id if lead_id else None,
                        'retailer_id': retailer.id if retailer_id else None,
                        ...
                        "ischeck": ischeck,
                        "type": "check-in",
                        "reason" : crec.description or '',
                        "lat": crec.checkin_lattitude,
                        "long": crec.checkin_longitude,
                        "portal_id" : crec.id,
                        'address': crec.reverse_location,
                        "journey_id" : '',
                        "mobile_id": crec.mobile_id,
                        "journey": journey,
                        'visit_type': crec.visit_type or '',
                        'location_type': crec.location_type or '',
                    }
    ...
    # @http.route('/wmvdapi/visit_counter', methods=["POST"], type='json', auth='user')
    @http.route('/wmvdapi/visit_counter', methods=["POST"], type='json', auth='public')
    def visit_counter(self, user_id=None, meeting_date=None):
        start = time.time()
        print("------------/wmvdapi/visit_counter -----------", user_id, meeting_date)
        emp_id = request.env['hr.employee'].sudo().search([('user_id', '=', user_id),('active', '=', True)], limit=1)
        # users_rec = request.env['res.users'].sudo().search([('id', '=', user_id),('active', '=', True)], limit=1).name

        domain1 = [('user_id', '=', user_id),('location_type','=','Headquarter'),('expense_date','=',meeting_date)]
        domain2 = [('user_id', '=', user_id),('location_type','=','Outstation'),('expense_date','=',meeting_date)]
        domain3 = [('user_id', '=', user_id),('visit_type','=','Retail'),('expense_date','=',meeting_date)]
        domain4 = [('user_id', '=', user_id),('visit_type','=','Project'),('expense_date','=',meeting_date)]

        todays_headquarter_visit = request.env['calendar.event'].sudo().search_count(domain1)
        todays_outstation_visit = request.env['calendar.event'].sudo().search_count(domain2)
        todays_retail_visit = request.env['calendar.event'].sudo().search_count(domain3)
        todays_project_visit = request.env['calendar.event'].sudo().search_count(domain4)

        min_retail_visits = emp_id.min_retail_visits
        min_project_visits = emp_id.min_project_visits
        min_headquarter_visits = emp_id.min_headquarter_visits
        min_outstation_visits = emp_id.min_outstation_visits

        response = {'user_id' : user_id or 0,
                    'user' :  emp_id.user_id.name or '',

                    'todays_retail_visit' : todays_retail_visit or 0,
                    'todays_project_visit' : todays_project_visit or 0,
                    'min_retail_visits' : min_retail_visits  or 10,
                    'min_project_visits' : min_project_visits  or 3,

                    'todays_headquarter_visit' : todays_headquarter_visit or 0,
                    'todays_outstation_visit' : todays_outstation_visit or 0,
                    'min_headquarter_visits' : min_headquarter_visits  or 10,
                    'min_outstation_visits' : min_outstation_visits  or 5,
                    }

        end = time.time()
        # print "------------- END visit_counter -------", end-start
        return {'success': response, 'error': None}
