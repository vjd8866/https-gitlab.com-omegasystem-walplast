odoo.define('document.document_kanban_tests', function (require) {
"use strict";
...
QUnit.module('document_kanban_tests.js', {
    beforeEach() {
        beforeEach(this);

        this.ORIGINAL_CREATE_XHR = documentKanbanController.prototype._createXHR;
        this.patchDocumentXHR = (mockedXHRs, customSend) => {
            documentKanbanController.prototype._createXhr = () => {
                const xhr = {
        ...
        Object.assign(this.data, {
            'document.document': {
                fields: {
                    ...
                    folder_id: {string: "Workspaces", type: 'many2one', relation: 'document.folder'},
                    ...
                    share_ids: {string: "Shares", type: "many2many", relation: 'document.share'},
                ...
                records: [
                    ...
                    {id: 3, name: 'gnap', file_size: 15000, lock_uid: 13, owner_id: 12, partner_id: 11,
                        public: false, res_id: 2, res_model: 'document.document', res_model_name: 'Task',
                    ...
                    {id: 6, name: 'pom', file_size: 70000, partner_id: 13,
                        public: true, res_id: 1, res_model: 'document.document', res_model_name: 'Document',
    ...
    afterEach() {
        documentKanbanController.prototype._createXHR = this.ORIGINAL_CREATE_XHR;
        afterEach(this);
    },
}, function () {
    QUnit.test('kanban basic rendering', async function (assert) {
        ...
        var kanban = await createdocumentView({
        ...
        assert.containsNone(kanban, '.o_document_selector_tags',
        ...
        assert.containsOnce(kanban.$('.o_content'), '> div.o_search_panel',
            "should have a 'document selector' column");
        assert.containsOnce(kanban, '.o_content > .o_kanban_view',
            "should have a 'classical kanban view' column");
        assert.hasClass(kanban.$('.o_kanban_view'), 'o_document_kanban_view',
            "should have classname 'o_document_kanban_view'");
        ...
        assert.containsOnce(kanban, '.o_content > .o_document_inspector',
        ...
        assert.containsOnce(kanban, '.o_cp_buttons button.o_document_kanban_url',
        ...
        assert.strictEqual(kanban.$('.o_cp_buttons button.o_document_kanban_request').text().trim(), 'Request',
        ...
        assert.strictEqual(kanban.$('.o_cp_buttons button.btn-secondary.o_document_kanban_share_domain').text().trim(), 'Share',
    ...
    QUnit.test('only visible selected records are kept after a reload', async function (assert) {
        ...
        var kanban = await createdocumentView({
            ...
            arch: '<kanban><templates><t t-name="kanban-box">' +
                ...
                '</t></templates></kanban>',
            archs: {
                "document.document,false,search": `
                    <search>
        ...
        assert.containsN(kanban, '.o_document_inspector_preview .o_document_preview', 3,
            "should show 3 document previews in the documentInspector");
        ...
        assert.containsOnce(kanban, '.o_document_inspector_preview .o_document_preview',
            "should show 1 document preview in the documentInspector");
        ...
        assert.containsOnce(kanban, '.o_document_inspector_preview .o_document_preview',
            "should show 1 document previews in the documentInspector");
    ...
    QUnit.test('selected records are kept when a button is clicked', async function (assert) {
        ...
        var kanban = await createdocumentView({
            ...
            mockRPC: function (route, args) {
                if (args.method === 'read' && args.model === 'document.document') {
            ...
            intercepts: {
                execute_action: function (ev) {
                    ...
                    self.data['document.document'].records[0].name = 'yop changed';
                    ev.data.on_closed();
                },
            },
        });
        ...
        assert.containsN(kanban, '.o_document_inspector_preview .o_document_preview', 3,
            "should still show 3 document previews in the documentInspector");
    ...
    QUnit.test('can share current domain', async function (assert) {
        ...
        // filter on 'task' in the documentSelector
        await testUtils.dom.click(
        ...
        await testUtils.dom.click(kanban.$buttons.find('.o_document_kanban_share_domain'));
        ...
        var kanban = await createdocumentView({
            ...
            intercepts: {
                do_action: function (ev) {
                    assert.deepEqual(ev.data.action, "document.action_url_form", "should open the URL form");
                },
            },
        });

        await testUtils.dom.click(kanban.$('.o_search_panel_category_value header:eq(1)'));
        await testUtils.dom.click(kanban.$buttons.find('button.o_document_kanban_url'));
    ...
    QUnit.test('can Request a file', async function (assert) {
        ...
        var kanban = await createdocumentView({
            ...
            intercepts: {
                do_action: function (ev) {
                    assert.deepEqual(ev.data.action, "document.action_request_form", "should open the Request form");
                },
            },
        });

        await testUtils.dom.click(kanban.$('.o_search_panel_category_value header:eq(1)'));
        await testUtils.dom.click(kanban.$buttons.find('button.o_document_kanban_request'));
    ...
    QUnit.test('document inspector with no document selected', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.o_search_panel_category_value header:eq(1)'));
        assert.strictEqual(kanban.$('.o_document_inspector_preview').text().replace(/\s+/g, ''),
            '_F1-test-description_', "should display the current workspace description");
        assert.strictEqual(kanban.$('.o_document_inspector_info .o_inspector_value:first').text().trim(),
            '5', "should display the correct number of document");
        assert.strictEqual(kanban.$('.o_document_inspector_info .o_inspector_value:nth(1)').text().trim(),
            '0.12 MB', "should display the correct size");

        kanban.destroy();
    });

    QUnit.test('document inspector with selected document', async function (assert) {
        assert.expect(5);
        ...
        assert.containsNone(kanban, '.o_document_inspector_info .o_selection_size',
            "should not display the number of selected document (because only 1)");
        assert.containsOnce(kanban, '.o_document_inspector_preview .o_document_preview',
            "should show a preview of the selected document");
        assert.hasClass(kanban.$('.o_document_inspector_preview .o_document_preview'), 'o_document_single_preview',
            "should have the 'o_document_single_preview' className");
        ...
        assert.strictEqual(kanban.$('.o_document_inspector_preview .o_selection_size').text().trim(),
            '2 document selected', "should display the correct number of selected document");
        assert.containsN(kanban, '.o_document_inspector_preview .o_document_preview', 2,
            "should show a preview of the selected document");

        kanban.destroy();
    });

    QUnit.test('document inspector limits preview to 4 document', async function (assert) {
        assert.expect(2);
        ...
        assert.strictEqual(kanban.$('.o_document_inspector_preview .o_selection_size').text().trim(),
            '5 document selected', "should display the correct number of selected document");
        assert.containsN(kanban, '.o_document_inspector_preview .o_document_preview', 4,
            "should only show a preview of 4 selected document");
    ...
    QUnit.test('document inspector shows selected records of the current page', async function (assert) {
        ...
        assert.containsNone(kanban, '.o_document_inspector_preview .o_document_preview',
            "should show no document preview in the documentInspector");
    ...
    QUnit.test('document inspector: document preview', async function (assert) {
        ...
        var kanban = await createdocumentView({
            ...
            mockRPC: function (route) {
                if (route === '/web/static/lib/pdfjs/web/viewer.html?file=/web/content/2?model%3Ddocument.document%26filename%3Dblip') {
        ...
        assert.containsOnce(kanban, '.o_document_split_pdf_area', "should have a pdf splitter");
    ...
    QUnit.test('document inspector: can delete records', async function (assert) {
        ...
        assert.containsN(kanban, '.o_document_inspector_preview .o_document_preview', 2,
            "should show 2 document previews in the documentInspector");

        await testUtils.dom.click(kanban.$('.o_document_inspector_info .o_inspector_delete'));
        await testUtils.nextTick();

        assert.containsNone(kanban, '.o_record_selected',
            "should have no selected record");
        assert.containsNone(kanban, '.o_document_inspector_preview .o_document_preview',
            "should show 0 document preview in the documentInspector");
    ...
    QUnit.test('document inspector: can archive records', async function (assert) {
        ...
        var kanban = await createdocumentView({
            ...
            arch: '<kanban><templates><t t-name="kanban-box">' +
                ...
                '</t></templates></kanban>',
            mockRPC: function (route, args) {
                if (route === "/web/dataset/call_kw/document.document/toggle_active") {
                    var documentIDS = args.args[0];
                    var records = this.data['document.document'].records;
        ...
        await testUtils.dom.click(kanban.$('.o_document_inspector_info .o_inspector_archive'));
    ...
    QUnit.test('document inspector: can share records', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.o_document_inspector_info .o_inspector_share'));
        ...
        var kanban = await createdocumentView({
            ...
            mockRPC: function (route, args) {
                if (args.method === 'toggle_lock') {
                    ...
                    var record = self.data['document.document'].records.find((d) => d.id === 1);
    ...
    QUnit.test('document inspector: document info with several document selected', async function (assert) {
        assert.expect(7);
    ...
    QUnit.test('document inspector: update document info with several document selected', async function (assert) {
        assert.expect(10);
    ...
    QUnit.test('document inspector: open resource', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.o_document_inspector .o_inspector_object_name'));

        kanban.destroy();
    });

    QUnit.test('document inspector: display tags of selected document', async function (assert) {
        assert.expect(4);
        ...
        assert.containsOnce(kanban, '.o_inspector_tag',
            "should display the common tags between the two selected document");
    ...
    QUnit.test('document inspector: unknown tags are hidden', async function (assert) {
        ...
        this.data['document.document'].records[0].tag_ids = [1, 2, 78];
    ...
    QUnit.test('document inspector: display rules of selected document', async function (assert) {
        assert.expect(6);
        ...
        assert.containsOnce(kanban, '.o_inspector_rule',
            "should display the common rules between the two selected document");
    ...
    QUnit.test('document inspector: locked by another user', async function (assert) {
        ...
        this.data['document.document'].records.push({
            id: 54,
    ...
    QUnit.test('document inspector: trigger rule actions on selected document', async function (assert) {
        assert.expect(3);
        ...
        var kanban = await createdocumentView({
            ...
            arch: '<kanban><templates><t t-name="kanban-box">' +
                ...
                '</t></templates></kanban>',
            mockRPC: function (route, args) {
                if (args.model === 'document.workflow.rule' && args.method === 'apply_actions') {
                    assert.deepEqual(args.args[0], [1],
    ...
    QUnit.test('document chatter: open and close chatter', async function (assert) {
        ...
        // open the chatter
        await testUtils.dom.click(kanban.$('.o_document_inspector .o_inspector_open_chatter'));
        ...
        assert.containsOnce(kanban, '.o_document_inspector:visible',
    ...
    QUnit.test('document chatter: fetch and display chatter messages', async function (assert) {
        ...
        this.data['document.document'].records[0].message_ids = [101, 102];
        this.data['mail.message'].records.push(
            { body: "Message 1", id: 101, model: 'document.document', res_id: 1 },
            { body: "Message 2", id: 102, model: 'document.document', res_id: 1 }
        );
        ...
        await afterNextRender(() => 
            testUtils.dom.click(kanban.$('.o_document_inspector .o_inspector_open_chatter'))
        );
    ...
    QUnit.test('document chatter: fetch and display followers', async function (assert) {
        ...
        this.data['document.document'].records[0].message_follower_ids = [301, 302];
    ...
    QUnit.test('document chatter: keep chatter open when switching between records', async function (assert) {
        ...
        this.data['document.document'].records[0].message_ids = [101, 102];
        this.data['mail.message'].records.push(
            { body: "Message on 'yop'", id: 101, model: 'document.document', res_id: 1 },
            { body: "Message on 'blip'", id: 102, model: 'document.document', res_id: 2 }
        );
    ...
    QUnit.test('document selector: render without facets & tags', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.o_search_panel_category_value header:eq(1)'));
        assert.strictEqual(kanban.$('.o_search_panel .o_document_selector_tags .o_search_panel_section_header').text().trim(),
    ...
    QUnit.test('document selector: render without related models', async function (assert) {
        ...
        assert.containsNone(kanban, '.o_search_panel .o_document_selector_tags .o_search_panel_section_header',
            "shouldn't have a 'tags' section");
        await testUtils.dom.click(kanban.$('.o_search_panel_category_value header:eq(1)'));
        assert.strictEqual(kanban.$('.o_search_panel .o_search_panel_section:nth-child(3) .o_search_panel_section_header').text().trim(),
            'Attached To', "should have an 'attached to' section");
    ...
    QUnit.test('document selector: selected tags are reset when switching between workspaces', async function (assert) {
        ...
        var kanban = await createdocumentView({
            ...
            arch: '<kanban><templates><t t-name="kanban-box">' +
                ...
                '</t></templates></kanban>',
            mockRPC: function (route, args) {
                if (route === '/web/dataset/search_read' && args.model === 'document.document') {
    ...
    QUnit.test('document Kanban color widget', async function (assert) {
        assert.expect(4);
        var kanban = await createdocumentView({
            View: documentKanbanView,
            model: 'document.document',
            ...
            arch: '<kanban><templates><t t-name="kanban-box">' +
                    '<div>' +
                        '<field name="tag_ids" widget="document_many2many_tags"/>' +
                    '</div>' +
                '</t></templates></kanban>',
        });
    ...
    QUnit.test('SearchPanel: can drag and drop in the search panel', async function (assert) {
        ...
        const kanban = await createdocumentView({
            ...
            data: this.data,
            mockRPC: function (route, args) {
                if (args.method === 'write' && args.model === 'document.document') {
                    assert.deepEqual(
                        args.args[0],
                        [1, 4, 2],
                    ...
                    assert.step('document.document/write');
                }
                return this._super.apply(this, arguments);
            },
            arch: `
        ...
        assert.verifySteps(['document.document/write']);
    ...
    QUnit.test('document: upload progress bars', async function (assert) {
        assert.expect(5);
        ...
        testUtils.file.dropFile(kanban.$('.o_document_view'), file);
    ...
    QUnit.test('document: upload replace bars', async function (assert) {
        assert.expect(5);

        const documentId = 23;
        this.data['document.document'].records = []; // reset incompatible setup
        this.data['document.document'].records.push({
            folder_id: 1,
            id: documentId,
            name: 'request',
        ...
        const ORIGINAL_PROMPT_FILE_INPUT = documentKanbanController.prototype._promptFileInput;
        documentKanbanController.prototype._promptFileInput = $input => testUtils.file.inputFiles($input[0], [file]);
        ...
        const kanban = await createdocumentView({
            ...
            arch: `
                <kanban><templates><t t-name="kanban-box">
                    <div draggable="true" class="oe_kanban_global_area o_document_attachment" data-id="${documentId}">
                        <field name="name"/>
                    </div>
                </t></templates></kanban>`,
        });

        await testUtils.dom.click(kanban.$('.o_document_attachment'));
        ...
        assert.containsOnce(kanban, '.o_document_attachment > .o_file_upload_progress_bar',
            "there should be a progress card");
        assert.containsOnce(kanban, '.o_document_attachment > .o_file_upload_progress_bar > .o_file_upload_progress_bar_value',
            "there should be a progress bar");
        assert.containsOnce(kanban, '.o_document_attachment > .o_file_upload_progress_bar > .o_upload_cross',
            "there should be cancel upload cross");

        kanban.destroy();
        documentKanbanController.prototype._promptFileInput = ORIGINAL_PROMPT_FILE_INPUT;
    });

    QUnit.test('document: upload multiple progress bars', async function (assert) {
        assert.expect(8);
        ...
        const $dropZone = kanban.$('.o_document_view');
    ...
    QUnit.test('document: notifies server side errors', async function (assert) {
        assert.expect(3);
    ...
    QUnit.test('document Kanban: displays youtube thumbnails', async function (assert) {
        assert.expect(1);
        ...
        assert.strictEqual(
            kanban.$('.o_document_inspector img, .o_document_single_preview .o_preview_available').attr('data-src'),
            "https://img.youtube.com/vi/Ayab6wZ_U1A/0.jpg",
            "the inspector should display the thumbnail of the youtube video");
        kanban.destroy();
    });

    QUnit.test('document List: multi selection', async function (assert) {
        assert.expect(4);
    ...
    QUnit.test('document List: selection using keyboard', async function (assert) {
        assert.expect(6);
    ...
    QUnit.test('document List: listview is not groupable', async function (assert) {
        assert.expect(1);
    ...
    QUnit.test('document: Versioning', async function (assert) {
        assert.expect(13);
        ...
        const kanban = await createdocumentView({
            ...
            async mockRPC(route, args) {
                ...
                if (args.model === 'document.document' && args.method === 'write') {
    ...
    QUnit.test("store and retrieve active category value", async function (assert) {
        ...
        const storageKey = "searchpanel_document.document_folder_id";
