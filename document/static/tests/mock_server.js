odoo.define('document.MockServer', function (require) {
'use strict';
...
MockServer.include({
    ...
    _performRpc: function (route) {
        if (route.indexOf('/document/image') >= 0 ||
    ...
    /**
     * Mocks the '_get_models' method of the model 'document.document'.
     *
     * @param {string} model
     * @param {any[]} domain
     */
    _mockGetModels(model, domain) {
        ...
        for (const group of groups) {
            if (!group.res_model) {
            ...
            } else if (group.res_model === 'document.document') {
                notAttached.push({
                    id: group.res_model,
                ...
                const { res_model_name } = this.data['document.document'].records.find(
    ...
    /**
     * Override to handle the specific case of model 'document.document'.
     *
     * @override
     */
    _mockSearchPanelSelectRange: function (model, [fieldName], kwargs) {
        if (model === 'document.document' && fieldName === 'folder_id') {
            const enableCounters = kwargs.enable_counters || false;
            const fields = ['display_name', 'description', 'parent_folder_id'];
            const records = this._mockSearchRead('document.folder', [[], fields], {});
        ...
        if (model === 'document.document') {
            if (fieldName === 'tag_ids') {
                ...
                if (folderId) {
                    ...
                    return {values: this.data['document.tag'].get_tags(domain, folderId), };
                } else {
                    return {values: [], };
                }
