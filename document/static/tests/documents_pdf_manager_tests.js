odoo.define('document.document_pdf_manager_tests', function (require) {
'use strict';

const PdfManager = require('document.component.PdfManager');
...
QUnit.module('document_pdf_manager_tests.js', {
    beforeEach() {
        utils.patch(PdfManager, 'document_pdf_manager_tests', {
            async _loadAssets() { },
            async _getPdf() {
                return {
    ...
    afterEach() {
        utils.unpatch(PdfManager, 'document_pdf_manager_tests');
    },
}, () => {
    QUnit.test('Pdf Manager basic rendering', async function (assert) {
        ...
        assert.containsOnce(pdfManager, '.o_document_pdf_manager_top_bar',
            "There should be one top bar");
        assert.containsOnce(pdfManager, '.o_document_pdf_page_viewer',
        ...
        assert.containsN(pdfManager, '.o_document_pdf_button_wrapper', 12,
    ...
    QUnit.test('Pdf Manager: page interactions', async function (assert) {
        ...
        await testUtils.dom.click($(pdfManager.el).find('.o_document_pdf_page_selector:nth(3)'));
    ...
    QUnit.test('Pdf Manager: drag & drop', async function (assert) {
        ...
        assert.containsOnce($(pdfManager.el).find('.o_document_pdf_page_frame:nth(2)'), '.o_pdf_name_display',
        ...
        $(pdfManager.el).find('.o_document_pdf_canvas_wrapper:nth(5)')[0].dispatchEvent(startEvent);

        const endEvent = new Event('drop', { bubbles: true, });
        endEvent.dataTransfer = dataTransfer;
        $(pdfManager.el).find('.o_document_pdf_canvas_wrapper:nth(1)')[0].dispatchEvent(endEvent);

        await testUtils.nextTick();

        assert.containsN(pdfManager, '.o_pdf_separator_activated', 4,
            "There should be 4 active separator");
        assert.containsNone($(pdfManager.el).find('.o_document_pdf_page_frame:nth(2)'), '.o_pdf_name_display',
