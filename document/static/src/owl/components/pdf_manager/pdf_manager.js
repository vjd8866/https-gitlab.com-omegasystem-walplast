odoo.define('document.component.PdfManager', function (require) {
'use strict';

const PdfGroupName = require('document.component.PdfGroupName');
const PdfPage = require('document.component.PdfPage');
...
class PdfManager extends owl.Component {
    ...
    constructor() {
        ...
        this.state = useState({
            ...
            // whether to archive the original document.
            archive: true,
    ...
    mounted() {
        ...
        for (const pdf_document of this.props.document) {
            this._addFile(pdf_document.name, {
                url: `/document/content/${pdf_document.id}`,
     ...
     * @param {String} name
     * @param {Object} param1
     * @param {number} [param1.documentId] the id of the `document.document` record.
    ...
    async _applyChanges(ruleId) {
        ...
        try {
            ...
            this.trigger('process-document', { documentIds, ruleId, exit });
            if (!exit) {
    ...
    _createPages({ fileId, name, pageCount }) {
        ...
        for (let pageNumber = 1; pageNumber <= pageCount; pageNumber++) {
            ...
            if (!groupLock) {
                ...
                groupLock = this.props.document.length > 1;
    ...
    async _loadAssets() {
        let asset;
        try {
            asset = await ajax.loadAsset('document.pdf_js_assets');
        } catch (error) {
    ...
    async _renderCanvas(page, { width, height }) {
        ...
        canvas.className = "o_document_pdf_canvas";
        canvas.width = width;
    ...
    async _sendChanges({ exit, ruleId } = {}) {
        ...
        return new Promise((resolve, reject) => {
            ...
            xhr.open('POST', '/document/pdf_split');
            ...
            const document = this.props.document[0];
    ...
    _onClickGlobalClose(ev) {
        ev.stopPropagation();
        this.trigger('process-document', { exit: true });
    }
    /**
     * @private
...
PdfManager.template = 'document.component.PdfManager';
