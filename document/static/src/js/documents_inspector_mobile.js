odoo.define('document.documentInspectorMobile', function (require) {
"use strict";

var config = require('web.config');

if (!config.device.isMobile) {
    return;
}

var documentInspector = require('document.documentInspector');

documentInspector.include({
    template: 'document.documentInspectorMobile',

    init: function() {
        this._super.apply(this, arguments);
        this.isMobile = true;
    },
    
    async start() {
        await this._super(...arguments);
        this.trigger_up('open_chatter');
    },
    //--------------------------------------------------------------------------
    // Public
    //--------------------------------------------------------------------------

    /**
     * Return the insternal state of the widget.
     * Add the 'open' status to the state.
     *
     * @override
     * @returns {Object}
     */
    getLocalState: function () {
        return _.extend({}, this._super.apply(this, arguments), {
            open: this.el.getAttribute('open') === '',
        });
    },
    /**
     * Restore the given state.
     *
     * @override
     * @param {Object} state
     * @param {integer} state.open the 'open' status to restore
     */
    setLocalState: function (state) {
        this._super.apply(this, arguments);
        if (state.open) {
            this.open();
        }
    },
    /**
     * Open the inspector.
     */
    open: function () {
        this.el.setAttribute('open', '');
    },
});

});
