odoo.define('document.controllerMixin', function (require) {
'use strict';

const documentInspector = require('document.documentInspector');
const DocumentViewer = require('document.DocumentViewer');
const { computeMultiSelection } = require('document.utils');
...
const documentControllerMixin = Object.assign({}, fileUploadMixin, {
    ...
    events: {
        'click .o_document_kanban_share_domain': '_onClickdocumentShareDomain',
        'click .o_document_kanban_upload': '_onClickdocumentUpload',
        'click .o_document_kanban_url': '_onClickdocumentUploadFromUrl',
        'click .o_document_kanban_request': '_onClickdocumentRequest',
        'dragleave .o_document_view': '_onDragleavedocumentView',
        'dragover .o_document_view': '_onDragoverdocumentView',
        'dragstart .o_document_draggable': '_onDragstartDocumentDraggable',
        'drop .o_document_view': '_onDropdocumentView',
    },
    custom_events: Object.assign({}, fileUploadMixin.custom_events, {
        ...
        o_close_chatter: '_ondocumentCloseChatter',
     ...
     */
    renderButtons($node) {
        this.$buttons = $(qweb.render('documentViews.buttons'));
        if ($node) {
    ...
    _getFileUploadRenderOptions() {
        ...
        return {
            ...
            targetCallback: fileUpload => {
                const $targetCard = this.$(`.o_document_attachment[data-id="${fileUpload.recordId}"]`);
     ...
     */
    _getFileUploadRoute() {
        return '/document/upload_attachment';
    },
    /**
     * @private
     * @param {Object[]} document
     * @return {Object[]} rules that are common to the given document.
     */
    _getRules(document) {
        const ruleIdsByRecord = document.map(record => record.available_rule_ids.res_ids);
    ...
    _makeChatterContainerProps() {
        return {
            // `document.document` has `mail.activity.mixin`
            hasActivities: this.modelName === 'document.document',
            hasFollowers: true,
    ...
    _makeDragIcon({ dataTransfer, lockedCount, draggedRecordIds }) {
        let dragIconContent;
         if (lockedCount > 0) {
            dragIconContent = _.str.sprintf(_t("%s document (%s locked)"), draggedRecordIds.length, lockedCount);
        } else {
            dragIconContent = _.str.sprintf(_t("%s document"), draggedRecordIds.length);
        }
        ...
        const $dragIcon = $(qweb.render('document.dragIcon', {
            dragIconContent,
        })).appendTo($('body'));
    ...
    async _renderChatter() {
        ...
        const $chatterContainer = $(qweb.render('document.ChatterContainer'));
        this.$('.o_content').addClass('o_chatter_open');
        if (this.$('.o_document_mobile_inspector').length) {
            this.$('.o_document_mobile_inspector').append($chatterContainer);
        } else {
            this.$('.o_content').append($chatterContainer);
        }
        const target = $chatterContainer[0].querySelector(':scope .o_document_chatter_placeholder');
        await this._chatterContainerComponent.mount(target);
    },
    /**
     * Renders and appends the document inspector sidebar.
     *
     * @private
     */
    async _renderdocumentInspector(state) {
        let localState;
        if (this._documentInspector) {
            localState = this._documentInspector.getLocalState();
            this._documentInspector.destroy();
        }
        this._documentInspector = new documentInspector(this, {
        ...
        this._isInspectorTagInputFocusOnMount = false;
        await this._documentInspector.insertAfter(this.renderer.$el);
        if (localState) {
            this._documentInspector.setLocalState(localState);
        }
    },
    /**
    ...
    /**
     * Override to render the document selector and inspector sidebars and
    ...
    async _update(state, { controllerState: { selectedRecordIds, }={}, }={}) {
        ...
        await this._renderdocumentInspector(state);
    },
    /**
    ...
    updateButtons() {
        ...
        this.$buttons.find('.o_document_kanban_upload').prop('disabled', !selectedFolderId);
        this.$buttons.find('.o_document_kanban_url').prop('disabled', !selectedFolderId);
        this.$buttons.find('.o_document_kanban_request').prop('disabled', !selectedFolderId);
        this.$buttons.find('.o_document_kanban_share_domain').prop('disabled', !selectedFolderId);
    },
    /**
    ...
    async _updateChatter() {
        ...
        if (this._selectedRecordIds.length === 1) {
            ...
            const target = chatterContainer.querySelector(':scope .o_document_chatter_placeholder');
            await this._chatterContainerComponent.mount(target);
        } else {
            this._closeChatter();
        }
     ...
     * @param {MouseEvent} ev
     */
    _onClickdocumentUploadFromUrl(ev) {
        ev.preventDefault();
        const context = this.model.get(this.handle, {raw: true}).getContext();
        this.do_action('document.action_url_form', {
            additional_context: {
                default_partner_id: context.default_partner_id || false,
                default_folder_id: this.searchModel.get('selectedFolderId'),
                default_tag_ids: [[6, 0, this.searchModel.get('selectedTagIds')]],
     ...
     * @param {DragEvent} ev
     */
    async _onDropdocumentView(ev) {
        if (!ev.originalEvent.dataTransfer.types.includes('Files')) {
            return;
        }
        ev.preventDefault();
        this.renderer.$el.removeClass('o_document_drop_over');
        this.$('.o_document_upload_text').remove();
    ...
    _onDownload(ev) {
        ...
        if (resIds.length === 1) {
            window.location = `/document/content/${resIds[0]}`;
        } else {
            session.get_file({
                url: '/document/zip',
                data: {
                    file_ids: resIds,
                    zip_name: `document-${moment().format('YYYY-MM-DD')}.zip`
                },
            });
        }
    },
    ...
    _onDragoverdocumentView(ev) {
        ...
        ev.preventDefault();
        this.renderer.$el.addClass('o_document_drop_over');
        if (this.$('.o_document_upload_text').length === 0) {
            this.$('.o_content').append($(qweb.render('document.uploadText')));
        }
    },
    /**
     * @private
    ...
    /**
     * Adds the selected document to the data of the drag event and
     * creates a custom drag icon to represent the dragged document.
     *
     * @private
     * @param {DragEvent} ev
     */
    _onDragstartDocumentDraggable(ev) {
        ...
        ev.originalEvent.dataTransfer.setData('o_document_data', JSON.stringify({
    ...
    async _onKanbanImageClicked(ev) {
        ev.stopPropagation();
        const document = ev.data.recordList;
        const recordId = ev.data.recordId;
        const rules = this._getRules(document);
        const documentViewer = new DocumentViewer(this, document, recordId, {
            openPdfManager: ev.data.openPdfManager,
            rules,
        });
        await documentViewer.appendTo(this.$('.o_document_view'));
    },
    /**
     * @private
     * @param {OdooEvent} ev
    ...
    /**
     * Save the changes done in the documentInspector and re-render the view.
     *
     * @private
     * @param {OdooEvent} ev
