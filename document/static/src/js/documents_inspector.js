odoo.define('document.documentInspector', function (require) {
'use strict';

/**
 * This file defines the documentInspector Widget, which is displayed next to
 * the KanbanRenderer in the documentKanbanView.
 */
...
const documentInspector = Widget.extend({
    template: 'document.documentInspector',
    custom_events: {
    ...
    init: function (parent, params) {
        ...
        this.nbdocument = params.state.count;
    ...
    /**
     * Compute the classes to use in documentInspector.previews template
     *
     * @param {Object} record
     * @return {String}
     */
    _computeClasses: function (record) {
        ...
        if (nbPreviews === 1) {
            classes.push("o_document_single_preview");
        }
        if ((recordData.isImage || recordData.isYouTubeVideo)) {
            classes.push("o_document_preview_image");
        } else {
            classes.push("o_document_preview_mimetype");
        }
    ...
    _renderField: function (fieldName, options) {
        ...
        const $row = $(qweb.render('document.documentInspector.infoRow'));

        // render the label
        const $label = $(qweb.render('document.documentInspector.fieldLabel', {
            icon: options.icon,
    ...
    _renderFields: function () {
        ...
        if (this.records.length > 0) {
            ...
            proms.push(this._renderField('folder_id', {
                icon: 'fa fa-folder o_document_folder_color',
                mode: 'edit',
            }));
        }
    ...
    async _renderHistory() {
        ...
        $(qweb.render('document.inspector.attachmentHistory', {
            attachments,
            str_to_datetime,
    ...
    _renderModel: function () {
        ...
        if (!resModelName || this.records[0].data.res_model === 'document.document') {
            return;
        }
        ...
        $modelContainer.append(qweb.render('document.documentInspector.resModel', options));
    },
    /**
     * @private
     */
    _renderRules: function () {
        ...
        for (const rule of this._rules) {
            if (this.records.length === 1 || !rule.limited_to_single_record) {
                const $rule = $(qweb.render('document.documentInspector.rule', rule));
    ...
    _renderTags: function () {
        ...
        for (const tag of commonTags) {
            if (tag) {
                ...
                const $tag = $(qweb.render('document.documentInspector.tag', tag));
    ...
    /**
     * Download the selected document (zipped if there are several document).
     *
     * @private
     */
     ...
     * which will be handled by the documentKanbanController.
     *
     * @private
     * @param {OdooEvent} ev
     */
