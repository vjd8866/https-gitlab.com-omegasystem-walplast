odoo.define('document.DocumentViewer', function (require) {
'use strict';
...
/**
 * This file defines the DocumentViewer for the document Kanban view.
 */
const documentDocumentViewer = DocumentViewer.extend(WidgetAdapterMixin, {
    template: "documentDocumentViewer",
    events: Object.assign({}, DocumentViewer.prototype.events, {
        'click .o_document_pdf_manager_button': '_onClickdocumentPdfManagerButton',
    }),
    custom_events: Object.assign({}, DocumentViewer.prototype.custom_events, {
        process_document: '_onProcessdocument',
     ...
     * @param {Object[]} param3.rules the workflow rules that can be applied to the document
     */
    init(parent, attachments, activeAttachmentID, { openPdfManager, rules } = {}) {
        this._super(...arguments);
        this.modelName = 'document.document';
        this._document = attachments;
    ...
    async start() {
        ...
        const isPdfOnly = this._document.every(record => record.mimetype === 'application/pdf');
        if (this._openPdfManager && isPdfOnly) {
            await this._renderPdfManager(this._document);
        }
    },
    ...
    async _renderPdfManager(document) {
     ...
     * @param {MouseEvent} ev
     */
    async _onClickdocumentPdfManagerButton(ev) {
        ev.stopPropagation();
