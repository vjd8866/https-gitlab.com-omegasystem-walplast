...
class ShareRoute(http.Controller):
    ...
    def binary_content(self, id, env=None, field='datas', share_id=None, share_token=None,
        ...
        record = env['document.document'].browse(int(id))
        filehash = None

        if share_id:
            share = env['document.share'].sudo().browse(int(share_id))
            record = share._get_document_and_check_access(share_token, [int(id)], operation='read')
    ...
    @http.route('/document/upload_attachment', type='http', methods=['POST'], auth="user")
    def upload_document(self, folder_id, ufile, document_id=False, partner_id=False, owner_id=False):
        ...
        if document_id:
            document = request.env['document.document'].browse(int(document_id))
            ufile = files[0]
            try:
            ...
            document = request.env['document.document'].with_context(allowed_company_ids=allowed_company_ids).create(vals_list)
            result['ids'] = document.ids

        return json.dumps(result)

    @http.route('/document/pdf_split', type='http', methods=['POST'], auth="user")
    def pdf_split(self, new_files=None, ufile=None, archive=False, vals=None):
        """Used to split and/or merge pdf document.

        The data can come from different sources: multiple existing document
        ...
        :param ufile: extra uploaded files that are not existing document
        :param archive: whether to archive the original document
        :param vals: values for the create of the new document.
        """
        ...
        document = request.env['document.document'].browse(document_ids)

        with ExitStack() as stack:
            files = request.httprequest.files.getlist('ufile')
            open_files = [stack.enter_context(io.BytesIO(file.read())) for file in files]

            # merge together data from existing document and from extra uploads
            ...
            # update new_files structure with the new indices from document
            for new_file in new_files:
                for page in new_file['new_pages']:
            ...
            new_document = document._pdf_split(new_files=new_files, open_files=open_files, vals=vals)

        # archive original document if needed
        if archive == 'true':
            document.write({'active': False})

        response = request.make_response(json.dumps(new_document.ids), [('Content-Type', 'application/json')])
        return response

    @http.route(['/document/content/<int:id>'], type='http', auth='user')
    def document_content(self, id):
        return self._get_file_response(id)

    @http.route(['/document/image/<int:id>',
                 '/document/image/<int:id>/<int:width>x<int:height>',
    ...
    def get_zip(self, file_ids, zip_name, token=None):
        ...
        env = request.env
        response = self._make_zip(zip_name, env['document.document'].browse(ids_list))
        if token:
    ...
    def share_download_all(self, access_token=None, share_id=None):
        ...
        env = request.env
        try:
            share = env['document.share'].sudo().browse(share_id)
            document = share._get_document_and_check_access(access_token, operation='read')
            if document:
                return self._make_zip((share.name or 'unnamed-link') + '.zip', document)
            else:
                return request.not_found()
        except Exception:
    ...
    def get_avatar(self, access_token=None, share_id=None):
        ...
        try:
            env = request.env
            share = env['document.share'].sudo().browse(share_id)
            if share._get_document_and_check_access(access_token, document_ids=[], operation='read') is not False:
    ...
    def upload_attachment(self, share_id, token, document_id=None, **kwargs):
        ...
        """
        share = http.request.env['document.share'].sudo().browse(share_id)
        if not share.can_upload or (not document_id and share.action != 'downloadupload'):
            return http.request.not_found()

        available_document = share._get_document_and_check_access(
        ...
        chatter_message = _('''<b> File uploaded by: </b> %s <br/>
                               ...
                               <a class="btn btn-primary" href="/web#id=%s&model=document.share&view_type=form" target="_blank">
                                  <b>%s</b>
                               </a>
                             ''') % (
        ...
        if document_id and available_document:
            if available_document.type != 'empty':
                return http.request.not_found()
            try:
                ...
                available_document.with_context(binary_field_real_user=http.request.env.user).write(write_vals)
                available_document.message_post(body=chatter_message)
        elif not document_id and available_document is not False:
            try:
                for file in request.httprequest.files.getlist('files'):
                    ...
                    document = request.env['document.document'].with_user(share.create_uid).with_context(binary_field_real_user=http.request.env.user).create(document_dict)
                    document.message_post(body=chatter_message)
                    if share.activity_option:
                        document.document_set_activity(settings_record=share)
    ...
    def share_portal(self, share_id=None, token=None):
        ...
        try:
            share = http.request.env['document.share'].sudo().browse(share_id)
            available_document = share._get_document_and_check_access(token, operation='read')
            if available_document is False:
                if share._check_token(token):
                    options = {
                        'expiration_date': share.date_deadline,
                        'author': share.create_uid.name,
                    }
                    return request.render('document.not_available', options)
                else:
                    return request.not_found()
            ...
            if share.type == 'ids' and len(available_document) == 1:
                options.update(document=available_document[0], request_upload=True)
                return request.render('document.share_single', options)
            else:
                options.update(all_button='binary' in [document.type for document in available_document],
                               document_ids=available_document,
                               request_upload=share.action == 'downloadupload' or share.type == 'ids')
                return request.render('document.share_page', options)
        except Exception:
