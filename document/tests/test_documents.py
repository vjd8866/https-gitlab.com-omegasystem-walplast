...
class TestCasedocument(TransactionCase):
    """ """
    def setUp(self):
        super(TestCasedocument, self).setUp()
        self.doc_user = self.env['res.users'].create({
            'name': 'Test user document',
            'login': 'document@example.com',
        })
        self.folder_a = self.env['document.folder'].create({
            'name': 'folder A',
        })
        self.folder_a_a = self.env['document.folder'].create({
            'name': 'folder A - A',
            'parent_folder_id': self.folder_a.id,
        })
        self.folder_b = self.env['document.folder'].create({
            'name': 'folder B',
        })
        self.tag_category_b = self.env['document.facet'].create({
            'folder_id': self.folder_b.id,
            'name': "categ_b",
        })
        self.tag_b = self.env['document.tag'].create({
            'facet_id': self.tag_category_b.id,
            'name': "tag_b",
        })
        self.tag_category_a = self.env['document.facet'].create({
            'folder_id': self.folder_a.id,
            'name': "categ_a",
        })
        self.tag_category_a_a = self.env['document.facet'].create({
            'folder_id': self.folder_a_a.id,
            'name': "categ_a_a",
        })
        self.tag_a_a = self.env['document.tag'].create({
            'facet_id': self.tag_category_a_a.id,
            'name': "tag_a_a",
        })
        self.tag_a = self.env['document.tag'].create({
            'facet_id': self.tag_category_a.id,
            'name': "tag_a",
        })
        self.document_gif = self.env['document.document'].create({
            'datas': GIF,
            'name': 'file.gif',
            'mimetype': 'image/gif',
            'folder_id': self.folder_b.id,
        })
        self.document_txt = self.env['document.document'].create({
            'datas': TEXT,
            'name': 'file.txt',
            'mimetype': 'text/plain',
            'folder_id': self.folder_b.id,
        })
        self.share_link_ids = self.env['document.share'].create({
            'document_ids': [(4, self.document_txt.id, 0)],
            'type': 'ids',
            'name': 'share_link_ids',
            'folder_id': self.folder_a_a.id,
        })
        self.share_link_folder = self.env['document.share'].create({
            'folder_id': self.folder_a_a.id,
            'name': "share_link_folder",
        })
        self.tag_action_a = self.env['document.workflow.action'].create({
            'action': 'add',
            'facet_id': self.tag_category_b.id,
            'tag_id': self.tag_b.id,
        })
        self.worflow_rule = self.env['document.workflow.rule'].create({
            ...
            'activity_type_id': self.env.ref('document.mail_document_activity_data_Inbox').id,
    ...
    def test_document_create_from_attachment(self):
        """
        Tests a document.document create method when created from an already existing ir.attachment.
        """
        ...
        self.assertEqual(document_a.res_model, 'document.document',
    ...
    def test_document_create_write(self):
        """
        Tests a document.document create and write method,
        document should automatically create a new ir.attachments in relevant cases.
        """
        document_a = self.env['document.document'].create({
    ...
    def test_document_rules(self):
        """
        Tests a document.workflow.rule
        """
        ...
        self.assertEqual(activity_gif.activity_type_id.id,
                         self.env.ref('document.mail_document_activity_data_Inbox').id,
                         "failed at activity data note from workflow create activity")
    ...
    def test_document_rule_display(self):
        """
        tests criteria of rules
        """

        self.workflow_rule_criteria = self.env['document.workflow.rule'].create({
        ...
        self.document_txt_criteria_a = self.env['document.document'].create({
        ...
        self.assertTrue(self.workflow_rule_criteria.id not in self.document_txt_criteria_a.available_rule_ids.ids,
                        "failed at document_workflow_rule unavailable rule")

        self.document_txt_criteria_b = self.env['document.document'].create({
        ...
        self.assertTrue(self.workflow_rule_criteria.id not in self.document_txt_criteria_b.available_rule_ids.ids,
                        "failed at document_workflow_rule unavailable rule")
        self.document_txt_criteria_c = self.env['document.document'].create({
            'datas': TEXT,
            'name': 'file.txt',
            'mimetype': 'text/plain',
            'folder_id': self.folder_a.id,
        ...
        self.assertTrue(self.workflow_rule_criteria.id in self.document_txt_criteria_c.available_rule_ids.ids,
                        "failed at document_workflow_rule available rule")

        self.document_txt_criteria_d = self.env['document.document'].create({
    ...
    def test_document_share_links(self):
        """
        ...
        action_folder = self.env['document.share'].create_share(vals)
        result_share_folder = self.env['document.share'].search([('folder_id', '=', self.folder_b.id)])
        result_share_folder_act = self.env['document.share'].browse(action_folder['res_id'])
        ...
        vals = {
            ...
            'activity_option': True,
            'activity_type_id': self.ref('document.mail_document_activity_data_tv'),
            'activity_summary': 'test by Folder with upload and activites',
            'activity_date_deadline_range': 4,
            'activity_date_deadline_range_type': 'days',
            'activity_user_id': self.env.user.id,
        }
        action_folder_with_upload = self.env['document.share'].create_share(vals)
        share_folder_with_upload = self.env['document.share'].browse(action_folder_with_upload['res_id'])
        ...
        self.assertEqual(share_folder_with_upload.activity_type_id.name, 'To validate',
                         'failed at activity type for upload document')
        ...
        action_document = self.env['document.share'].create_share(vals)
        result_share_document_act = self.env['document.share'].browse(action_document['res_id'])

        # Expiration date
        self.assertEqual(result_share_document_act.state, 'expired', "failed at share_link expired")
    ...
    def test_request_activity(self):
        ...
        document_1 = self.env['document.document'].search([('request_activity_id', '=', activity.id)], limit=1)
        document_2 = self.env['document.document'].search([('request_activity_id', '=', activity_2.id)], limit=1)
    ...
    def test_versioning(self):
        """
        Tests the versioning/history of document
        """
        document = self.env['document.document'].create({'datas': GIF, 'folder_id': self.folder_b.id})
    ...
    def test_write_mimetype(self):
        """
        Tests the consistency of document' mimetypes
        """
        document = self.env['document.document'].create({'datas': GIF, 'folder_id': self.folder_b.id})
    ...
    def test_cascade_delete(self):
        """
        Makes sure that document are unlinked when their attachment is unlinked.
        """
        document = self.env['document.document'].create({'datas': GIF, 'folder_id': self.folder_b.id})
    ...
    def test_is_favorited(self):
        user = new_test_user(self.env, "test user", groups='document.group_document_user')
        document = self.env['document.document'].create({'datas': GIF, 'folder_id': self.folder_b.id})
    ...
    def test_create_from_message(self):
        ...
        share = self.env['document.share'].create({
        ...
        attachment_document = self.env['document.document'].browse(attachment.res_id)
