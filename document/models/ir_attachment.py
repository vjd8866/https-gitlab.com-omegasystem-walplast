...
class IrAttachment(models.Model):
    ...
    def _create_document(self, vals):
        """
        Implemented by bridge modules that create new document if attachments are linked to
        their business models.

        :param vals: the create/write dictionary of ir attachment
        :return True if new document are created
        """
        # Special case for document
        if vals.get('res_model') == 'document.document' and vals.get('res_id'):
            document = self.env['document.document'].browse(vals['res_id'])
        ...
        if model is not None and res_id and issubclass(type(model), self.pool['document.mixin']):
            vals_list = [
    ...
    def create(self, vals):
        ...
        # the context can indicate that this new attachment is created from document, and therefore
