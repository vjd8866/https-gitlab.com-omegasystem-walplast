...
class WorkflowActionRule(models.Model):
    _name = "document.workflow.rule"
    _description = "A set of condition and actions which will be available to all attachments matching the conditions"
    _order = "sequence"

    domain_folder_id = fields.Many2one('document.folder', string="Related Workspace", required=True, ondelete='cascade')
    ...
    required_tag_ids = fields.Many2many('document.tag', 'required_tag_ids_rule_table', string="Required Tags")
    excluded_tag_ids = fields.Many2many('document.tag', 'excluded_tag_ids_rule_table', string="Excluded Tags")
    ...
    tag_action_ids = fields.One2many('document.workflow.action', 'workflow_rule_id', string='Set Tags')
    folder_id = fields.Many2one('document.folder', string="Move to Workspace")
    ...
    def create_record(self, document=None):
        """
        ...
        :param document: the list of the document of the selection
    ...
    def apply_actions(self, document_ids):
        ...
        :param document_ids: the list of document to apply the action.
        ...
        """
        document = self.env['document.document'].browse(document_ids)
        ...
        if self.activity_option and self.activity_type_id:
            document.document_set_activity(settings_record=self)

        if self.create_model:
            return self.with_company(document.company_id).create_record(document=document)
...
class WorkflowTagAction(models.Model):
    ...
    workflow_rule_id = fields.Many2one('document.workflow.rule', ondelete='cascade')
    ...
    facet_id = fields.Many2one('document.facet', string="Category")
    tag_id = fields.Many2one('document.tag', string="Tag")
    ...
    def execute_tag_action(self, document):
        if self.action == 'add' and self.tag_id.id:
            ...
            faceted_tags = self.env['document.tag'].search([('facet_id', '=', self.facet_id.id)])
            if faceted_tags.ids:
