...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        if attachment_ids:
            for record in self:
                document = self.env['document.document'].search([('request_activity_id', '=', record.id)], limit=1)
                if document and not document.attachment_id:
                    self.env['document.document'].search([('attachment_id', '=', attachment_ids[0])]).unlink()
                    if not feedback:
    ...
    def create(self, values):
        ...
        if activity_type.category == 'upload_file' and activity.res_model != 'document.document':
