...
class Document(models.Model):
    ...
    owner_id = fields.Many2one('res.users', default=lambda self: self.env.user.id, string="Owner",
                               tracking=True)
    available_rule_ids = fields.Many2many('document.workflow.rule', compute='_compute_available_rules',
    ...
    create_share_id = fields.Many2one('document.share', help='Share used to create this document')
    ...
    def _get_models(self, domain):
        ...
        :param domain: the domain of the read_group on document.
        ...
        for group in groups:
            ...
            elif res_model == 'document.document':
                not_attached.append({
                    'id': res_model,
    ...
    def _compute_available_rules(self):
        ...
        rules = self.env['document.workflow.rule'].sudo().search(rule_domain)
        for rule in rules:
            domain = []
            ...
            document_ids = self.env['document.document'].search(subset)
    ...
    def document_set_activity(self, settings_record=None):
        """
    ...
    def access_content(self):
        ...
        if self.url:
            ...
            action['url'] = '/document/content/%s' % self.id
        return action
    ...
    def create_share(self):
        ...
        return self.env['document.share'].create_share(vals)
    ...
    def toggle_lock(self):
        ...
        Members of the group document.group_document_manager and the superuser can unlock the file regardless.
        """
        self.ensure_one()
        if self.lock_uid:
            if self.env.user == self.lock_uid or self.env.is_admin() or self.user_has_groups(
                    'document.group_document_manager'):
                self.lock_uid = False
        else:
    ...
    def _compute_is_locked(self):
        for record in self:
            record.is_locked = record.lock_uid and not (
                    ...
                    self.user_has_groups('document.group_document_manager'))
    ...
    def create(self, vals):
        ...
        if (attachment and not attachment.res_id and (not attachment.res_model or attachment.res_model == 'document.document')):
            attachment.with_context(no_document=True).write({'res_model': 'document.document', 'res_id': new_record.id})
        return new_record
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            if record.attachment_id:
                ...
                res_model = vals.get('res_model', record.res_model or 'document.document')
                res_id = vals.get('res_id') if vals.get('res_model') else record.res_id if record.res_model else record.id
                if res_model and res_model != 'document.document' and not self.env[res_model].browse(res_id).exists():
                    record.res_model = res_model = 'document.document'
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        if field_name == 'folder_id':
            ...
            available_folders = self.env['document.folder'].search([])
            ...
            DocumentFolder = self.env['document.folder'].sudo().with_context(hierarchical_naming=False)
    ...
    def _get_processed_tags(self, domain, folder_id):
        ...
        """
        tags = self.env['document.tag']._get_tags(domain, folder_id)
        facets = list(OrderedDict.fromkeys([tag['group_id'] for tag in tags]))
        facet_colors = self.env['document.facet'].FACET_ORDER_COLORS
        for tag in tags:
