...
class DocumentFolder(models.Model):
    ...
    parent_folder_id = fields.Many2one('document.folder',
                                       string="Parent Workspace",
                                       ondelete="cascade",
                                       help="A workspace will inherit the tags of its parent workspace")
    name = fields.Char(required=True, translate=True)
    description = fields.Html(string="Description", translate=True)
    children_folder_ids = fields.One2many('document.folder', 'parent_folder_id', string="Sub workspaces")
    document_ids = fields.One2many('document.document', 'folder_id', string="document")
    sequence = fields.Integer('Sequence', default=10)
    share_link_ids = fields.One2many('document.share', 'folder_id', string="Share Links")
    facet_ids = fields.One2many('document.facet', 'folder_id',
                                string="Tag Categories",
                                help="Tag categories defined for this workspace")
    group_ids = fields.Many2many('res.groups',
        string="Write Groups", help='Groups able to see the workspace and read/create/edit its document.')
    read_group_ids = fields.Many2many('res.groups', 'document_folder_read_groups',
        string="Read Groups", help='Groups able to see the workspace and read its document without create/edit rights.')

    user_specific = fields.Boolean(string="Own document Only",
                                   help="Limit Read Groups to the document of which they are owner.")
    ...
    def _compute_action_count(self):
        read_group_var = self.env['document.workflow.rule'].read_group(
    ...
    def action_see_actions(self):
        return {
            'name': _('Actions'),
            'res_model': 'document.workflow.rule',
    ...
    def _compute_document_count(self):
        read_group_var = self.env['document.document'].read_group(
