...
class documenthare(models.Model):
    _name = 'document.share'
    _inherit = ['mail.thread', 'mail.alias.mixin']
    _description = 'document Share'

    folder_id = fields.Many2one('document.folder', string="Workspace", required=True)
    name = fields.Char(string="Name")

    access_token = fields.Char(required=True, default=lambda x: str(uuid.uuid4()), groups="document.group_document_user")
    ...
    # type == 'ids'
    document_ids = fields.Many2many('document.document', string='Shared document')
    # type == 'domain'
    domain = fields.Char()
    ...
    def _get_document(self, document_ids=None):
        """
        :param list[int] document_ids: limit to the list of document to fetch.
        :return: recordset of the document that can be accessed by the create_uid based on the settings
        of the share link.
        """
        self.ensure_one()
        limited_self = self.with_user(self.create_uid)
        document = limited_self.env['document.document']
        ...
        return document.search(search_domain)

    def _get_writable_document(self, document):
        """

        :param document:
        :return: the recordset of document for which the create_uid has write access
        False only if no write right.
        """
        self.ensure_one()
        try:
            # checks the rights first in case of empty recordset
            document.with_user(self.create_uid).check_access_rights('write')
        except exceptions.AccessError:
            return False
        return document.with_user(self.create_uid)._filter_access_rules('write')
    ...
    def _get_document_and_check_access(self, access_token, document_ids=None, operation='write'):
        """
        :param str access_token: the access_token to be checked with the share link access_token
        :param list[int] document_ids: limit to the list of document to fetch and check from the share link.
        :param str operation: access right to check on document (read/write).
        :return: Recordset[document.document]: all the accessible requested document
        False if it fails access checks: False always means "no access right", if there are no document but
        ...
        if self.state == 'expired':
            return False
        document = self._get_document(document_ids)
        if operation == 'write':
            return self._get_writable_document(document)
        else:
            return document
    ...
    def _alias_get_creation_values(self):
        values = super(documenthare, self)._alias_get_creation_values()
    ...
    def create(self, vals):
        ...
        share = super(documenthare, self).create(vals)
        return share
    ...
    def create_share(self, vals):
        ...
        view_id = self.env.ref('document.share_view_form_popup').id
        return {
